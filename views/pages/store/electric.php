<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="utf-8" />
  <meta content="width=device-width, initial-scale=1.0" name="viewport" />

  <title>Cheetarr | Electric-Guitar</title>
  <meta content="" name="description" />
  <meta content="" name="keywords" />

  <?php require('link.php');?>
</head>

<body>
<?php require('navbar.php');?>
  <!--Product-->
  <section id="prod" class="prod section-bg">
    <div class="container" data-aos="fade-up">
      <div class="section-title">
        <p class="text-center">ELECTRIC GUITAR</p>
        <p class="text-center fw-light text-lowercase" style="font-size: 15px">
          Shop our extensive collection of acoustic guitars from some of the top guitar brands in the world. When you
          purchase your guitar from Cheetarr, you will receive superior service from our staff. Questions? Call us at
          336-236-2009
          or email contact@cheetarr.com and we'll be happy to help you!
        </p>
      </div>

      <div class="row">
        <div class="container">
          <div class="row">
            <div class="col-md-3 col-sm-6">
              <div class="product-grid5">
                <div class="product-image">
                  <a href="#">
                    <img class="pic-1" src="../static/assets/img/elektrik/ele 1.png" />
                  </a>
                  <a href="#" class="select-options" data-name="p-1"></i>QUICK VIEW</a>
                </div>
                <div class="product-content text-center">
                  <h3 class="title text-uppercase"><a href="#">FENDER MODERN PLAYER HSS HONEY BURST USED</a></h3>
                  <div class="price">$549.00</div>
                </div>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <div class="product-grid5">
                <div class="product-image">
                  <a href="#">
                    <img class="pic-1" src="../static/assets/img/elektrik/ele 2.png" />
                  </a>
                  <a href="#" class="select-options" data-name="p-2"></i>QUICK VIEW</a>
                </div>
                <div class="product-content text-center">
                  <h3 class="title text-uppercase"><a href="#">SUHR MATEUS ASATO SIGNATURE SHELL ANTIQUE USED</a></h3>
                  <div class="price">$3,350.00</div>
                </div>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <div class="product-grid5">
                <div class="product-image">
                  <a href="#">
                    <img class="pic-1" src="../static/assets/img/elektrik/ele 3.png" />
                  </a>
                  <a href="#" class="select-options" data-name="p-3"></i>QUICK VIEW</a>
                </div>
                <div class="product-content text-center">
                  <h3 class="title text-uppercase"><a href="#">GIBSON SG TRIBUTE NATURAL WALNUT</a></h3>
                  <div class="price">$1,199.00</div>
                </div>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <div class="product-grid5">
                <div class="product-image">
                  <a href="#">
                    <img class="pic-1" src="../static/assets/img/elektrik/ele 4.png" />
                  </a>
                  <a href="#" class="select-options" data-name="p-4"></i>QUICK VIEW</a>
                </div>
                <div class="product-content text-center">
                  <h3 class="title text-uppercase"><a href="#">GIBSON LES PUAL STUDIO</a></h3>
                  <div class="price">$1,199.00</div>
                </div>
              </div>
            </div>
          </div>
          <!---->
          <!--bwh-->
          <div class="row">
            <div class="col-md-3 col-sm-6">
              <div class="product-grid5">
                <div class="product-image">
                  <a href="#">
                    <img class="pic-1" src="../static/assets/img/elektrik/ele 5.png" />
                  </a>
                  <a href="#" class="select-options" data-name="p-5"></i>QUICK VIEW</a>
                </div>
                <div class="product-content text-center">
                  <h3 class="title text-uppercase"><a href="#">CHARVEL PRO MOD JOE DUPLANTIER SD2 HH SATIN WHITE</a>
                  </h3>
                  <div class="price">$699.99</div>
                </div>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <div class="product-grid5">
                <div class="product-image">
                  <a href="#">
                    <img class="pic-1" src="../static/assets/img/elektrik/ele 6.png" />
                  </a>
                  <a href="#" class="select-options" data-name="p-6"></i>QUICK VIEW</a>
                </div>
                <div class="product-content text-center">
                  <h3 class="title text-uppercase"><a href="#">JACKSON PRO SL3R - MIRROR</a></h3>
                  <div class="price">$3,350.00</div>
                </div>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <div class="product-grid5">
                <div class="product-image">
                  <a href="#">
                    <img class="pic-1" src="../static/assets/img/elektrik/ele 7.png" />
                  </a>
                  <a href="#" class="select-options" data-name="p-7"></i>QUICK VIEW</a>
                </div>
                <div class="product-content text-center">
                  <h3 class="title text-uppercase"><a href="#">CHARVEL PRO-MOD SAN DIMAS STYLE 1 HH FR M</a></h3>
                  <div class="price">$1,049.00</div>
                </div>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <div class="product-grid5">
                <div class="product-image">
                  <a href="#">
                    <img class="pic-1" src="../static/assets/img/elektrik/ele 8.png" />
                  </a>
                  <a href="#" class="select-options" data-name="p-8"></i>QUICK VIEW</a>
                </div>
                <div class="product-content text-center">
                  <h3 class="title text-uppercase"><a href="#">JACKSON SLXDX - MULTI CAMO</a></h3>
                  <div class="price">$999.00</div>
                </div>
              </div>
            </div>
          </div>
          <hr />
        </div>
      </div>
    </div>
  </section>
  <!-- End Product Section -->

  <!--Quick view-->
  <div class="products-preview">
    <div class="preview" data-target="p-1">
      <i class="fas fa-times"></i>
      <img src="../static/assets/img/elektrik/ele 1.png" alt="" style="height: 35rem;" />
      <h3>FENDER MODERN PLAYER HSS HONEY BURST USED</h3>
      <div class="price">$799.00</div>
      <hr>
      <div class="buttons">
        <a href="#" class="cart">ADD TO CART</a>
      </div>
      <p>This one-of-a-kind J-45 Studio will turn heads. Handcrafted with a Sitka spruce top and walnut back and sides.
        The thin body depth improves underarm comfort, while the flatter fingerboard allows for better playing.
        Available in both Antique Natural and Walnut Burst finishes, as well as lefty! Finished with an active
        under-saddle piezo pickup for convenient studio quality plug and play anywhere! </p>
    </div>
    <div class="preview" data-target="p-2">
      <i class="fas fa-times"></i>
      <img src="../static/assets/img/elektrik/ele 2.png" alt="" style="height: 35rem;" />
      <h3>SUHR MATEUS ASATO SIGNATURE SHELL ANTIQUE USED</h3>
      <div class="price">$749.00</div>
      <hr>
      <div class="buttons">
        <a href="#" class="cart">ADD TO CART</a>
      </div>
      <p>This one-of-a-kind J-45 Studio will turn heads. Handcrafted with a Sitka spruce top and walnut back and sides.
        The thin body depth improves underarm comfort, while the flatter fingerboard allows for better playing.
        Available in both Antique Natural and Walnut Burst finishes, as well as lefty! Finished with an active
        under-saddle piezo pickup for convenient studio quality plug and play anywhere! </p>
    </div>
    <div class="preview" data-target="p-3">
      <i class="fas fa-times"></i>
      <img src="../static/assets/img/elektrik/ele 3.png" alt="" style="height: 35rem;" />
      <h3>GIBSON SG TRIBUTE NATURAL WALNUT</h3>
      <div class="price">$1,199.00</div>
      <hr>
      <div class="buttons">
        <a href="#" class="cart">ADD TO CART</a>
      </div>
      <p>This one-of-a-kind J-45 Studio will turn heads. Handcrafted with a Sitka spruce top and walnut back and sides.
        The thin body depth improves underarm comfort, while the flatter fingerboard allows for better playing.
        Available in both Antique Natural and Walnut Burst finishes, as well as lefty! Finished with an active
        under-saddle piezo pickup for convenient studio quality plug and play anywhere! </p>
    </div>
    <div class="preview" data-target="p-4">
      <i class="fas fa-times"></i>
      <img src="../static/assets/img/elektrik/ele 4.png" alt="" style="height: 35rem;" />
      <h3>GIBSON LES PUAL STUDIO</h3>
      <div class="price">$1,199.00</div>
      <hr>
      <div class="buttons">
        <a href="#" class="cart">ADD TO CART</a>
      </div>
      <p>This one-of-a-kind J-45 Studio will turn heads. Handcrafted with a Sitka spruce top and walnut back and sides.
        The thin body depth improves underarm comfort, while the flatter fingerboard allows for better playing.
        Available in both Antique Natural and Walnut Burst finishes, as well as lefty! Finished with an active
        under-saddle piezo pickup for convenient studio quality plug and play anywhere! </p>
    </div>
    <!--bwh-->
    <div class="preview" data-target="p-5">
      <i class="fas fa-times"></i>
      <img src="../static/assets/img/elektrik/ele 5.png" alt="" style="height: 35rem;" />
      <h3>CHARVEL PRO MOD JOE DUPLANTIER SD2 HH SATIN WHITE</h3>
      <div class="price">$699.99</div>
      <hr>
      <div class="buttons">
        <a href="#" class="cart">ADD TO CART</a>
      </div>
      <p>This one-of-a-kind J-45 Studio will turn heads. Handcrafted with a Sitka spruce top and walnut back and sides.
        The thin body depth improves underarm comfort, while the flatter fingerboard allows for better playing.
        Available in both Antique Natural and Walnut Burst finishes, as well as lefty! Finished with an active
        under-saddle piezo pickup for convenient studio quality plug and play anywhere! </p>
    </div>
    <div class="preview" data-target="p-6">
      <i class="fas fa-times"></i>
      <img src="../static/assets/img/elektrik/ele 6.png" alt="" style="height: 35rem;" />
      <h3>JACKSON PRO SL3R - MIRROR</h3>
      <div class="price">$3,350.00</div>
      <hr>
      <div class="buttons">
        <a href="#" class="cart">ADD TO CART</a>
      </div>
      <p>This one-of-a-kind J-45 Studio will turn heads. Handcrafted with a Sitka spruce top and walnut back and sides.
        The thin body depth improves underarm comfort, while the flatter fingerboard allows for better playing.
        Available in both Antique Natural and Walnut Burst finishes, as well as lefty! Finished with an active
        under-saddle piezo pickup for convenient studio quality plug and play anywhere! </p>
    </div>
    <div class="preview" data-target="p-7">
      <i class="fas fa-times"></i>
      <img src="../static/assets/img/elektrik/ele 7.png" alt="" style="height: 35rem;" />
      <h3>CHARVEL PRO-MOD SAN DIMAS STYLE 1 HH FR M</h3>
      <div class="price">$1,049.00</div>
      <hr>
      <div class="buttons">
        <a href="#" class="cart">ADD TO CART</a>
      </div>
      <p>This one-of-a-kind J-45 Studio will turn heads. Handcrafted with a Sitka spruce top and walnut back and sides.
        The thin body depth improves underarm comfort, while the flatter fingerboard allows for better playing.
        Available in both Antique Natural and Walnut Burst finishes, as well as lefty! Finished with an active
        under-saddle piezo pickup for convenient studio quality plug and play anywhere! </p>
    </div>
    <div class="preview" data-target="p-8">
      <i class="fas fa-times"></i>
      <img src="../static/assets/img/elektrik/ele 8.png" alt="" style="height: 35rem;" />
      <h3>JACKSON SLXDX - MULTI CAMO</h3>
      <div class="price">$999.00</div>
      <hr>
      <div class="buttons">
        <a href="#" class="cart">ADD TO CART</a>
      </div>
      <p>This one-of-a-kind J-45 Studio will turn heads. Handcrafted with a Sitka spruce top and walnut back and sides.
        The thin body depth improves underarm comfort, while the flatter fingerboard allows for better playing.
        Available in both Antique Natural and Walnut Burst finishes, as well as lefty! Finished with an active
        under-saddle piezo pickup for convenient studio quality plug and play anywhere! </p>
    </div>
  </div>
  <!---->
  <?php require('footer.php');?>

  <div id="preloader"></div>
  <a href="#" class="back-to-top d-flex align-items-center justify-content-center"><i
      class="bi bi-arrow-up-short"></i></a>

      <?php require('js.php');?>
</body>

</html>